...
GCS_RUNS_ROOT="${GCS_RUNS_ROOT:-gs://omois-earth-world-model-phase2-20260320-11728/earth_world_model/runs/paper2_mixed_curriculum_suite}"
...
RUN_YEARLY_CONTROLS="${RUN_YEARLY_CONTROLS:-1}"
RUN_MIXED_MAIN="${RUN_MIXED_MAIN:-1}"
RUN_MIXED_METHOD_ABLATIONS="${RUN_MIXED_METHOD_ABLATIONS:-1}"
...
run_one() {
  ...
  local local_run_root="/tmp/ewm_paper2_mixed_curriculum_gpu/${run_id}"
...
if [[ "$RUN_MIXED_MAIN" == "1" ]]; then
  run_one \
    "mixed_ssl4eo_yearly_${YEARLY_TRAIN_MAX_SAMPLES}_stagec_conv2d_1024" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_stagec_conv2d_1024_mixed_ssl4eo_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"

  run_one \
    "mixed_ssl4eo_yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_mixed_ssl4eo_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"
fi

if [[ "$RUN_MIXED_METHOD_ABLATIONS" == "1" ]]; then
  run_one \
    "mixed_ssl4eo_yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_no_time_gap" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_mixed_ssl4eo_no_time_gap_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"

  run_one \
    "mixed_ssl4eo_yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_joint_sensor" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_mixed_ssl4eo_joint_sensor_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"

  run_one \
    "mixed_ssl4eo_yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_no_cross_sensor_loss" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_mixed_ssl4eo_no_cross_sensor_loss_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"
fi
