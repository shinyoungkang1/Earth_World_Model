...
if [[ "$BOOTSTRAP_PREP_VM" == "1" ]]; then
  ...
  gcloud compute ssh "$PREP_VM_NAME" \
    --project="$PROJECT_ID" \
    --zone="$ZONE" \
    --command="bash -lc 'if ! python3 -m pip --version >/dev/null 2>&1; then \
  ...
  gcloud compute ssh "$PREP_VM_NAME" \
    ...
    --command="bash -lc 'cd $(quote_for_bash "$REMOTE_REPO_ROOT") && \
      env \
        ...
        HLS_GCS_URI=$(quote_for_bash "$HLS_GCS_URI") \
        HLS_NAME=$(quote_for_bash "$HLS_NAME") \
        HLS_LOCAL_ROOT=$(quote_for_bash "$HLS_LOCAL_ROOT") \
        HLS_INDEX_PATH=$(quote_for_bash "$HLS_INDEX_PATH") \
        HLS_INDEX_RELATIVE_PATH=$(quote_for_bash "$HLS_INDEX_RELATIVE_PATH") \
        HLS_TRANSFER_BACKEND=$(quote_for_bash "$HLS_TRANSFER_BACKEND") \
        HLS_BUILD_OBS_EVENTS=$(quote_for_bash "$HLS_BUILD_OBS_EVENTS") \
        HLS_OBS_EVENTS_PARQUET=$(quote_for_bash "$HLS_OBS_EVENTS_PARQUET") \
        HLS_OBS_EVENTS_METADATA_JSON=$(quote_for_bash "$HLS_OBS_EVENTS_METADATA_JSON") \
        HLS_MAX_SAMPLES=$(quote_for_bash "$HLS_MAX_SAMPLES") \
        HLS_MIN_QUALITY_SCORE=$(quote_for_bash "$HLS_MIN_QUALITY_SCORE") \
        RESOLVE_MISSING_HLS_DATETIMES_FROM_CHIP=$(quote_for_bash "$RESOLVE_MISSING_HLS_DATETIMES_FROM_CHIP") \
        REQUIRE_EXISTING_HLS_CHIP_PATHS=$(quote_for_bash "$REQUIRE_EXISTING_HLS_CHIP_PATHS") \
        ...
        YEARLY_METADATA_FLUSH_SIZE=$(quote_for_bash "$YEARLY_METADATA_FLUSH_SIZE") \
        YEARLY_GCS_DOWNLOAD_WORKERS=$(quote_for_bash "$YEARLY_GCS_DOWNLOAD_WORKERS") \
        YEARLY_GCS_DOWNLOAD_TIMEOUT_SEC=$(quote_for_bash "$YEARLY_GCS_DOWNLOAD_TIMEOUT_SEC") \
        YEARLY_GCS_DOWNLOAD_RETRIES=$(quote_for_bash "$YEARLY_GCS_DOWNLOAD_RETRIES") \
        YEARLY_GCS_TRANSFER_BACKEND=$(quote_for_bash "$YEARLY_GCS_TRANSFER_BACKEND") \
        SSL4EO_WORKERS=$(quote_for_bash "$SSL4EO_WORKERS") \
        TOTAL_WORKER_BUDGET=$(quote_for_bash "$TOTAL_WORKER_BUDGET") \
