...
GCS_RUNS_ROOT="${GCS_RUNS_ROOT:-gs://omois-earth-world-model-phase2-20260320-11728/earth_world_model/runs/paper2_ablation_suite}"
...
RUN_CORE_MATRIX="${RUN_CORE_MATRIX:-1}"
RUN_METHOD_ABLATIONS="${RUN_METHOD_ABLATIONS:-1}"

run_one() {
  local run_id="$1"
  local config_path="$2"
  local local_data_root="$3"
  local local_run_root="/tmp/ewm_paper2_ablation_gpu/${run_id}"
...
if [[ "$RUN_CORE_MATRIX" == "1" ]]; then
  ...
  run_one \
    "ssl4eo_${SSL4EO_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024" \
    "$PROJECT_ROOT/earth_world_model/configs/ssl4eo_zarr_trainval_400_vjepa21_staged_rope_dense_conv3d_1024_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$SSL4EO_NAME"
fi

if [[ "$RUN_METHOD_ABLATIONS" == "1" ]]; then
  run_one \
    "yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_no_time_gap" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_no_time_gap_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"

  run_one \
    "yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_joint_sensor" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_joint_sensor_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"

  run_one \
    "yearly_${YEARLY_TRAIN_MAX_SAMPLES}_staged_rope_conv3d_1024_no_cross_sensor_loss" \
    "$PROJECT_ROOT/earth_world_model/configs/dense_temporal_index_pilot400_vjepa21_staged_rope_conv3d_1024_no_cross_sensor_loss_cuda_gpu_vm.yaml" \
    "$DATA_ROOT_BASE/$YEARLY_NAME"
fi
